var status_ques='';
var path_img="/file/dungchung/quiz_tate/images/";
var msg_kid='Bạn phải là <a target="_blank" href="2138-chuong-trinh-hoc-tieng-anh-tre-em-online-goldenkids.html"/*tpa=http://www.thuviengiaoduc.org/huong-dan/2138-chuong-trinh-hoc-tieng-anh-tre-em-online-goldenkids.html*/>Thành viên Goldenkids</a> của thuviengiaoduc.org mới được học tiếp bài học này.';
...
audio_ctrl = {
	...
	gold:'http://www.thuviengiaoduc.org/file/dungchung/libAudio/gold/goldenkids.mp3',
...
$(document).ready(function(){
	...
	*/
	...
	$('.qz_box img').click(function(){
		if ($(this).attr('src').indexOf('copy_bt-3.png'/*tpa=http://www.thuviengiaoduc.org/file/dungchung/quiz_tate/js/copy_bt.png*/)<0){
	...
	*/
	...
	$('.qz_outline_item').bind('click',function(){			
		...
		$('.qz_ouclose').live('click',function(){
	...
	if(ok==1){
		...
		switch (type){
			case 'Matching':{
				...
				$('.box_curr_'+t+' .stt_mat_drop').each(function(i){	
					...
					if(parseInt($(this).attr('droped'))==arr[5][inx_m]){
						...
						$('.box_curr_'+t+' .stt_mat_drag[drag='+arr[5][inx_m]+']').after('<div class="qz_st4_number qz_st4_number_r qz_correct_cil" style="position:absolute; top:'+top+'">'+(inx_dr+1)+'</div>');
					}else{
						inx_dr=$('.box_curr_'+t+' .stt_mat_drop[droped='+arr[5][inx_m]+']').index('.box_curr_'+t+' .stt_mat_drop');
						$(this).prev().addClass('qz_incorrect_cil');
						$('.box_curr_'+t+' .stt_mat_drag[drag='+$(this).attr('droped')+']').after('<div class="qz_st4_number qz_st4_number_r qz_incorrect_cil" style="position:absolute; top:'+top+'">'+(inx_dr+1)+'</div>');
					}					
				});			
				if(dem==countChild){				
					total[t]++;
			...
			case 'Multiple choice':{			
				...
				var ind=$('.box_curr_'+t+' .qz_multiple').index('.box_current_'+t+' .qz_multiple');	
				if(val==arr[2][ind]){
					total[t]++;
					d=1;					
			...
			case 'Sequence':{				
				...
				$('.box_curr_'+t+' .qz_sort_drop').each(function(i){					
					...
					if(parseInt($(this).attr('droped'))==arr[6][inx_s]){
						...
						text_true=$('.box_curr_'+t+' .qz_sort_drop[droped='+arr[6][inx_s]+']').index('.box_curr_'+t+' .qz_sort_drop');
						console.log(text_true);
						$(this).addClass('qz_incor');	
						$('.box_current_'+t+' .qz_index_sort_item:eq('+inx_s+')').addClass('qz_incor');		
					}
					$('.box_curr_'+t+' .qz_index_sort_item:eq('+text_true+')').text((i+1));	
				});					
				if(dem==countChild){
					total[t]++;
					d=1;
...
function qzOutline(cmd){		
	...
	$('.box_current_'+t+' .scroll_outline').tinyscrollbar();
}
//bai giang
var title_bg='';
var type='';
	...
	if(ok==1){
		...
		switch (type){
			case 'Matching':{
				...
				$('.bg_curr_'+t+' .stt_mat_drop').each(function(i){	
					...
					if(parseInt($(this).attr('droped'))==arr[5][inx_m]){
						...
						dem++;
						$(this).prev().addClass('qz_correct_cil');												
						$('.bg_curr_'+t+' .stt_mat_drag[drag='+arr[5][inx_m]+']').after('<div class="qz_st4_number qz_st4_number_r qz_correct_cil" style="position:absolute; top:'+top+'">'+(inx_dr+1)+'</div>');
					}else{
						inx_dr=$('.bg_curr_'+t+' .stt_mat_drop[droped='+arr[5][inx_m]+']').index('.bg_curr_'+t+' .stt_mat_drop');
						$(this).prev().addClass('qz_incorrect_cil');
						$('.bg_curr_'+t+' .stt_mat_drag[drag='+$(this).attr('droped')+']').after('<div class="qz_st4_number qz_st4_number_r qz_incorrect_cil" style="position:absolute; top:'+top+'">'+(inx_dr+1)+'</div>');
					}					
				});			
				if(dem==countChild){				
					total[t]++;
			...
			case 'Multiple choice':{				
				...
				var ind=$('.bg_curr_'+t+' .qz_multiple').index('.bg_current_'+t+' .qz_multiple');	
				if(val==arr[2][ind]){
					total[t]++;
					d=1;
					$('.bg_current_'+t+' .qz_st_anstatus').html('Your answer : <span class="qz_cor">Correct </span>');	
